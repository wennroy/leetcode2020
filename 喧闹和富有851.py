# -*- coding: utf-8 -*-
"""
Created on Tue Aug 11 16:11:02 2020
...
class Node:
    def __init__(self, val = -1, point = list()):
        self.val = val
        self.point = point
...
class Solution:
    def loudAndRich(self, richer, quiet):
        ...
        for item in richer:
            poor = item[1]
            rich = item[0]
            if index_list[poor].val == -1:
                index_list[poor] = Node(poor)
            if index_list[rich].val == -1:
                index_list[rich] = Node(rich)
            index_list[poor].point = index_list[poor].point + [index_list[rich]]
        ...
        for node in index_list:
            print(node.val,showval(node.point))
            
        order1 = [quiet.index(i) for i in sorted(quiet)]
        ans = []
        record = {}
        def traversal(root):
            if root.val in record.keys():
                richers.append(record[root.val])
                return
            richers.append(root.val)
            if root.point == []:
                return
            else:
                for next_root in root.point:
                    traversal(next_root)
...
print(Solution.loudAndRich(None,richer,quiet))
